...
/////////////////////////// CERTIFICATE StudentSol
  ...
  // Can the student receive a certificate?
  result = await academyCertification.validateStudent(StudentSol, {from: StudentOther});
  console.log("\n Can the student receive a certificate? ", result);   

  listNameInfo = await masterName.listNameInfo();
  console.log("masterName.listNameInfo: ", listNameInfo);
  
  for (i = 0; i < listNameInfo.length; i++) {
    console.log(i, listNameInfo[i]);
    if (listNameInfo[i].owner == StudentSol) {
      studentName = listNameInfo[i].name;
      break;
    }
  }
  console.log("listNameInfo studentName:", studentName);

  //Certificate StudentSol
  className = "Devs 2021-01";
  result = await academyCertification.registerCertificate(StudentSol, studentName, className, {from: StudentSol});
